...
public class PDFUtils {
  ...
  public static void drawPdfPath(
      VectorGraphics2D graphics,
      String desc,
    ...
    for (int pcnt = 0; pcnt < segments.size(); pcnt++) {
      if ((lcpr > 0) && (segments.get(pcnt)[0] == 1.0)) {
        ...
        graphics.fill(
            new Ellipse2D.Double(
                segments.get(pcnt)[3] * scale, segments.get(pcnt)[4] * scale, lcpr, lcpr));
      }
      if ((qcpr > 0) && (segments.get(pcnt)[0] == 2.0)) {
        ...
        graphics.fill(
            new Ellipse2D.Double(
                segments.get(pcnt)[3] * scale, segments.get(pcnt)[4] * scale, qcpr, qcpr));
        graphics.fill(
            new Ellipse2D.Double(
                segments.get(pcnt)[5] * scale, segments.get(pcnt)[6] * scale, qcpr, qcpr));
        graphics.draw(
            new Line2D.Double(
                segments.get(pcnt)[1] * scale,
                segments.get(pcnt)[2] * scale,
                segments.get(pcnt)[3] * scale,
                segments.get(pcnt)[4] * scale));
        graphics.draw(
            new Line2D.Double(
                segments.get(pcnt)[3] * scale,
                segments.get(pcnt)[4] * scale,
                segments.get(pcnt)[5] * scale,
                segments.get(pcnt)[6] * scale));
